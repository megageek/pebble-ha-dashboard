...
This repo is for a **Pebble smartwatch watchface that displays data from a linked Home Assistant instance** (e.g. sensor states, temperatures, switch/light status), plus a few always-available local channels (time, date, battery, steps). A basic scaffold exists at the repo root (`package.json`, `wscript`, `src/c/main.c`, emery-only) built around a channel/slot/template system, plus a Clay config page (`src/pkjs/`) for choosing the active template and per-slot channel assignment — see "Architecture" below. Not yet built: `src/pkjs/index.js`'s Home Assistant half (it currently only wires the Clay config bridge; the WebSocket/HA event relay described below is still to come), and gabbro support. Follow the skill below for build/test workflow.
...
`src/pkjs/` exists but today only implements the **config bridge**, not the HA WebSocket relay described below — those are separate, independently-added pieces of `index.js`.

- **`src/pkjs/config.js`** is the Clay form definition: a `TEMPLATE_INDEX` select (values `0`/`1`, matching `s_templates[]`) and five `SLOT_0_CHANNEL`…`SLOT_4_CHANNEL` selects (values `0`–`4`, matching `ChannelIndex`). Both templates happen to have exactly 5 slots, so one fixed set of 5 dropdowns covers either template without needing Clay's conditional/dynamic-field support — slots beyond what the active template uses are simply ignored on the watch side.
- **`src/pkjs/index.js`** wires `showConfiguration` → `clay.generateUrl()` and `webviewclosed` → `clay.getSettings(e.response)` sent via `Pebble.sendAppMessage()`. `pebble-clay` is an npm dependency (`package.json`/`node_modules`, `package-lock.json` committed for reproducibility); webpack (already used by `pebble build` for pkjs bundling) pulls it and `config.js` into `build/pebble-js-app.js` automatically — no wscript changes needed.
- **`package.json` `messageKeys`** are declared as a plain array (`TEMPLATE_INDEX`, `SLOT_0_CHANNEL`…`SLOT_4_CHANNEL`); the build generates numeric key values and a `MESSAGE_KEY_*` global per key (`build/src/message_keys.auto.c` + `build/include/message_keys.auto.h`, regenerated from a clean build — stale caches can leave the header empty, so `pebble clean` if `MESSAGE_KEY_*` symbols go missing after editing `messageKeys`). These are **runtime `uint32_t` variables, not `#define` macros** in this SDK version, so they can't be used to initialize a `static const` array — use a plain (non-static) local array if you need to iterate them by index, as `inbox_received_callback()` does for the 5 slot keys.
- **C side**: `s_active_template` and `s_slot_channel_override[5]` (`-1` = "use the template's default") replace the old compile-time `ACTIVE_TEMPLATE` constant. `load_config()` reads both from persistent storage (`persist_read_int`, keys `PERSIST_KEY_TEMPLATE`/`PERSIST_KEY_SLOT_BASE+i`) on `window_load`, falling back to defaults (template 0, no overrides) on first run or out-of-range values. `inbox_received_callback()` validates and `persist_write_int()`s any `TEMPLATE_INDEX`/`SLOT_n_CHANNEL` tuples it finds, then — if anything actually changed — tears down and rebuilds the slot layers immediately via `destroy_active_layout()`/`load_active_layout()`, so a config save applies live without relaunching the watchface. `app_message_register_inbox_received()` is called before `app_message_open()` in `init()`, per the standard AppMessage ordering requirement.
- **Testing without a phone**: `pebble send-app-message --emulator emery --uint <TEMPLATE_INDEX_key>=1 <SLOT_0_CHANNEL_key>=3` (numeric keys from `build/js/message_keys.json`) simulates what pkjs sends after Clay closes, without needing the actual webview — useful since the emulator has no phone to render Clay's HTML in. **Multiple key=value pairs must go after a single `--uint` flag, not repeated `--uint` flags** — `--uint 10000=1 --uint 10001=3` silently drops the first pair (argparse overwrite, not a merge); `--uint 10000=1 10001=3` sends both in one AppMessage.
...
- **Channel → slot mapping is runtime config, already built.** See the "Clay config page" section above — `src/pkjs/config.js`/`index.js` already persist template choice + per-slot channel assignment via `TEMPLATE_INDEX`/`SLOT_n_CHANNEL` AppMessage keys. What's still missing is the HA side: a real remote channel (beyond `CHANNEL_HA_PLACEHOLDER`) needs its own `messageKeys` (e.g. `CHANNEL_N_VALUE`/`CHANNEL_N_LABEL`) and C-side handling in `inbox_received_callback()`, separate from the config keys.
