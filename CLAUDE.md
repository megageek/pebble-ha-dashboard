...
- **Template** — a function returning, given the window bounds, an array of `SlotSpec { rect, size_class, channel }`. Two exist: `build_template_0()` (hero=`CHANNEL_TIME`, medium=`CHANNEL_DATE`, 3 small rows=`CHANNEL_BATTERY`/`CHANNEL_STEPS`/`CHANNEL_HA_PLACEHOLDER`) and `build_template_1()` (hero=`CHANNEL_BATTERY` shown as `"100%"` text since HERO always renders text, 4 small rows=`CHANNEL_TIME`/`CHANNEL_DATE`/`CHANNEL_STEPS`/`CHANNEL_HA_PLACEHOLDER`), proving slots vary in size, count, and channel assignment between templates, and that the same channel (e.g. battery) renders differently — bar vs. text-with-unit — purely because of the slot it lands in. `s_templates[]` is the array of builder function pointers; `ACTIVE_TEMPLATE` (a `#define`, currently `0`) picks which one loads. Slot→channel assignment is still hardcoded inside each template function; once the Clay config page exists, template choice and per-slot channel assignment both become user-configurable rather than fixed by array index/constant.
